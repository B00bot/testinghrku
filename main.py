...
def secret_message(msg):
    bot.sendMessage(msg.chat_id, "Мой создатель любит тебя")

@bot.message_handler(commands=['help'])
def help_message(msg):
    bot.sendMessage(msg.chat_id, "Если хочешь узнать тайну, отправь /secret Если нужны доказательства - отправь /proof Если грустно - отправь /grustno")

@bot.message_handler(commands=['proof'])
def proof_message(msg):
    randomstick = random.randint(0, 10)
    pic = stickers[randomstick]
    bot.sendMessage(msg.chat_id, "Создатель просил передать...")
    bot.sendSticker(msg.chat_id, pic);

@bot.message_handler(commands=['grustno'])
def grustno(msg):
    pic=open('s1200.jpeg', 'rb')
    bot.send_photo(msg.chat_id, pic);
    bot.sendMessage(msg.chat_id, "Ни грустииии")
